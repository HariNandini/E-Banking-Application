...
useEffect(() => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if(formValidation()){
  axios.post('http://localhost:8080/api/user/setorresetpin',cardpinInfo,
  {
    headers: {
    ...
    .then((response) => {
      ...
      window.alert("Successfully pin set.");
  ...
  return (
    ...
    overflow: 'hidden',padding:"30px"}}>
            <div className='row' >
               <div className='card col-md-6 offset-md-3 offset-md-3' style={{'marginTop': '130px', marginBottom: '270px'}}>
